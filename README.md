...
Back-end for Ilumeo-Point, a solution to mark the points of the employees.
...
npm run dev:seed
```
After running the 'seed', a user with the code '2xsso9' is created. Use this code to log in.
...
Set `POSTGRES_HOST` to `ilumeo-postgres-development` for `.env.development` and `ilumeo-postgres-test` for `.env.test` file. It is the name of the postgres container inside docker-compose file. Docker Compose will start the postgres container for you, create the database and host alias for you.
